...
export default function slackin ({
  ...
  app.post('/invite', json(), (req, res, next) => {
    ...
    const captcha_callback = (err, resp) => {
      ...
      if (err) {
        ...
        if(resp.body.success){
          ...
          let adminChanId = slack.getChannelId('admins');

          message({ token, org, email, username, channel: adminChanId }, err => {
            if (err) {
            ...
            res
            .status(200)
            .json({ msg: 'WOOT. Someone will send an invite to your email soon!' })
          });
    ...
    captcha_callback(null, {body: {success: true}});
