...
export default function slackin ({
  ...
  app.get('/', (req, res) => {
    ...
    let page = dom('html',
      ...
      splash({ coc, path, css, name, org, logo, channels, active, total, gcaptcha_sitekey, message_channel })
    )
    res.type('html')
    ...
    const captcha_callback = (err, resp) => {
      ...
      if (err) {
        ...
        if(resp.body.success){
          ...
          let adminChanId = slack.getChannelId(message_channel);

          let data = { token, org, email };
          let inviteFunc;
          if (message_channel) {
            data.channel = adminChanId;
          ...
          inviteFunc(data, err => {
            ...
            let msg = message_channel ? 'WOOT. Someone will send an invite to your email soon!' : 'WOOT. Check your email!';
  ...
  app.get('/iframe/dialog', (req, res) => {
    ...
    let page = dom('html',
      dom("script src=https://www.google.com/recaptcha/api.js"),
      splash({ coc, path, name, org, channels, active, total, large, iframe: true, gcaptcha_sitekey, message_channel })
    )
    res.type('html')
    res.send(page.toHTML())
  })
