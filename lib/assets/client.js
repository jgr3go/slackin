...
body.addEventListener('submit', function (ev){
  ...
  invite(channel ? channel.value : null, coc && coc.checked ? 1 : 0, email.value, username.value, gcaptcha_response.value, function (err, msg){
    if (err) {
...
function invite (channel, coc, email, username, gcaptcha_response_value, fn){
  request
