...
export default function splash ({ path, name, org, coc, logo, active, total, channels, large, iframe, gcaptcha_sitekey, message_channel }){
  ...
  let div = dom('.splash',
    ...
    dom('form id=invite',
      ...
      dom('input.form-item type=email name=email placeholder=you@yourdomain.com '
        + (!iframe ? 'autofocus' : '')),
      dom('input.form-item type=text name=username placeholder="reddit username"'),
      dom('br'),
      ...
      dom('button.loading', message_channel ? 'Request an Invite' : 'Get my Invite')
    ),
    dom('p.signin',
      'or ',
